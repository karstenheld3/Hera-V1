...
1. **IDE Docs** (preferred) - https://docs.windsurf.com/windsurf/models
2. **UI Screenshot Capture** (fallback) - when docs are outdated or unavailable

## Method A: IDE Docs (Recommended)
...
### Step 2: Model Reads Screenshots

The model reads each fullscreen screenshot from `../.tools/_screenshots/`.
Extracts model names and costs from the popup visible in each image.

### Step 3: Stop When List Wraps

When the model sees duplicate models (list wrapped around), extraction is complete.
